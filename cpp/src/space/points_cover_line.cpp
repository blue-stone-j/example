/*
a std::set storing points and each point has a small range of influence that "expands" toward its neighbors, expansion
distance is 5.
*/
...
//Calculates the left and right range of influence based on neighbors and the distance threshold.
void computeRanges(const std::set<double>& points, double threshold, std::vector<Range>& ranges) {
    ...
    for (; it != points.end(); ++it) {
        ...
        double left_dist = (it == points.begin()) ? threshold : std::min(*it - *prev, threshold);
