...
int main(int argc, char **argv)
{
  ...
  ros::NodeHandle nh("~"); // 局部句柄

  // Do parameter stuff.
  /*参数设定，可以读入我们在yaml中设定的参数*/
  GroundSegmentationParams params;
  nh.param("n_bins", params.n_bins, params.n_bins); //(，传入的变量，默认值)
  ...
  if (nh.getParam("r_min", r_min)) { params.r_min_square = r_min * r_min; }
  if (nh.getParam("r_max", r_max)) { params.r_max_square = r_max * r_max; }
  if (nh.getParam("max_fit_error", max_fit_error)) { params.max_error_square = max_fit_error * max_fit_error; }
