...
class SpaceFilter
{
 ...
 private:
  ...
  bool lateral_removal_;  // whether filter out points that locate outside boundary
  bool vertical_removal_; // whether filter out points that too high or too low vertically

  double left_distance_;  // horizonal boundary
  double right_distance_; // horizonal boundary
  double below_distance_; // height boundary
  double above_distance_; // height boundary

  void VelodyneCallback(const sensor_msgs::PointCloud2::Ptr &in_sensor_cloud_ptr);

  // filter out points that locate outside horizonal boundary(keep points in lane, ignore points outside lane)
  void KeepLanes(const pcl::PointCloud<pcl::PointXYZ>::Ptr in_cloud_ptr, pcl::PointCloud<pcl::PointXYZ>::Ptr out_cloud_ptr,
                 float in_left_lane_threshold, float in_right_lane_threshold);

  // filter out points that too high or too low vertically
  void ClipCloud(const pcl::PointCloud<pcl::PointXYZ>::Ptr in_cloud_ptr, pcl::PointCloud<pcl::PointXYZ>::Ptr out_cloud_ptr,
                 float in_min_height, float in_max_height);
};

SpaceFilter::SpaceFilter() :
  node_handle_("~") // 声明为局部句柄
{
